...
import type { MemorialPageData, RSVPStats } from '@/types'

// For now, we'll use mock data. In production, this fetches from Supabase.
async function getMemorialData(slug: string): Promise<MemorialPageData | null> {
  // Mock data for Reg Fulmer
  if (slug === 'reg-fulmer') {
    return {
      memorial: {
        id: '1',
        ...
        dob: '1942-11-26',
        dod: '2025-12-10',
        hero_photo_url: null, // Add photo URL when available
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const data = await getMemorialData(slug)
  ...
  return {
    title: data.memorial.page_title,
    description: data.memorial.short_bio,
    openGraph: {
      title: data.memorial.page_title,
      description: data.memorial.short_bio,
      type: 'website',
...
export default async function MemorialPage({ params }: PageProps) {
  ...
  const pageUrl = `https://memorial.regfulmer.au/m/${slug}`
  ...
  return (
    <main className="min-h-screen pb-20 md:pb-0">
      ...
      <QuickActionsBar
        ...
        pageUrl={pageUrl}
        pageName={data.memorial.page_title}
      />
      ...
      {data.service_event && (
        <ServiceSection
          serviceEvent={data.service_event}
          memorialName={data.memorial.preferred_name}
        />
      )}
      ...
      {data.service_event && (
        <RSVPSection
          ...
          serviceEventId={data.service_event.id}
          stats={data.rsvp_stats}
        />
      )}

      {/* Livestream */}
      {data.service_event?.livestream_enabled && (
        <LivestreamSection serviceEvent={data.service_event} />
      )}

      {/* Travel & stay */}
      {data.travel_tips.length > 0 && data.service_event && (
        <TravelSection
          tips={data.travel_tips}
          venueName={data.service_event.venue_name}
          googleMapsUrl={data.service_event.google_maps_url}
        />
      )}

      {/* Voice agent */}
      <VoiceAgentSection
        config={data.voice_config}
        memorialName={data.memorial.preferred_name}
      />
      ...
      {/* Footer */}
      <Footer memorialName={data.memorial.full_name} />

      {/* AI Chatbot */}
      <AIChatbot memorialId={data.memorial.id} />
    </main>
  )
}
